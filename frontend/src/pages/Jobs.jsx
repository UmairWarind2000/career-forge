...
useEffect(() => {
  liveJobsAPI.getCategories().then(r => setCategories(r.data.categories));
  gapAPI.getScore().then(r => setUserScore(r.data)).catch(() => {});
  fetchJobs();
}, []);

    const fetchJobs = async (customFilters = null, pageNum = 1) => {
        setLoading(true);
        try {
            const f = customFilters || filters;
            const res = await liveJobsAPI.search({ ...f, page: pageNum });
